...
def load_base_model_and_tokenizer(model_name='unsloth/Qwen2.5-0.5B-Instruct-bnb-4bit', max_seq_length=256):
    ...
    model, tokenizer = unsloth.FastLanguageModel.from_pretrained(model_name, max_seq_length, load_in_4bit = True)
...
def attach_lora_adapters(model, r=8, lora_alpha=16, target_modules=None):
    ...
    return unsloth.FastLanguageModel.get_peft_model(model, r=r, target_modules=target_modules, lora_alpha=lora_alpha)
